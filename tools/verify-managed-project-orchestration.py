...
def verify_managed_context(project_root: Path, expected_repository: str) -> list[dict[str, Any]]:
    ...
    manifest_path = project_root / ".ai/manifest.yaml"
...
def base_payload(repository_head: str) -> dict[str, Any]:
    return {
        ...
        "capabilities": {
            "verify_managed_context": "AVAILABLE",
        ...
        "tasks": [
            {
                ...
                "status": "COMPLETE",
                "priority": 100,
                "objective": "Recover real managed-project identity and durable context",
                "scope": ".ai/",
            },
            {
                "id": "verify_managed_context",
                "status": "PLANNED",
                "priority": 90,
                "dependencies": ["recover_project"],
                "objective": "Verify managed-project repository evidence and context contract",
                "scope": ".ai/",
            },
            {
                ...
                "status": "PLANNED",
                "priority": 80,
                "dependencies": ["verify_managed_context"],
                "objective": "Prove durable checkpoint and safe resume semantics",
                "scope": ".ai/ORCHESTRATION-CHECKPOINT.json",
            },
        ],
    }
...
def main() -> int:
    ...
    second = orchestrator.orchestrate(payload | {
        "completed_iterations": 1,
        "latest_runtime_outcome": {
            "task_id": "verify_managed_context",
            "status": "COMPLETE",
    ...
    final = orchestrator.orchestrate(payload | {
        "tasks": completed_tasks,
        "completed_iterations": 2,
        "latest_runtime_outcome": {
            "task_id": "checkpoint_resume",
            "status": "COMPLETE",
            "verification": "VERIFIED",
            "evidence": [
                {"source": "p13-checkpoint", "kind": "resume", "status": same_head["status"]},
                {"source": "p13-checkpoint", "kind": "head-drift", "status": changed_head["status"]},
            ],
        },
    })
    ...
    print(json.dumps({
        "status": "PASS",
        "protocol": "P13-MANAGED-PROJECT-PROOF-v1",
        "managed_project": args.expected_repository,
        "repository_head": head,
        "sequence": [
            first["runtime_handoff"]["work_unit"]["id"],
            second["runtime_handoff"]["work_unit"]["id"],
            final["decision"],
        ],
        "resume_same_head": same_head["status"],
        "resume_changed_head": changed_head["status"],
