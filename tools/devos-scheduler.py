...
def run_iteration(payload: dict[str, Any], project_root: Path, state_path: Path, checkpoint: Path | None = None) -> dict[str, Any]:
    ...
    return {"scheduler_version":"P12-SCHEDULER-v2","status":result.get("status","FAILED"),"recovery":recovery,"iteration":1,"runtime_loop":result,"execution":"DELEGATE_TO_EXISTING_RUNTIME","authorization":"UNCHANGED"}
...
def run_batch(payloads: list[dict[str, Any]], project_root: Path, state_path: Path, checkpoint: Path | None = None, max_iterations: int = 1) -> dict[str, Any]:
    ...
    for payload in payloads:
        if len(results) >= max_iterations: break
...
def main() -> int:
    ...
    args = parser.parse_args(); data=json.loads(args.input.read_text(encoding="utf-8")); result = run_batch(data, args.project_root, args.state, args.checkpoint, args.max_iterations) if isinstance(data, list) else run_iteration(data, args.project_root, args.state, args.checkpoint)
