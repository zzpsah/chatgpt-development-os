#!/usr/bin/env python3
"""DevOS bounded autonomous loop: OI -> controller -> handoff -> runtime -> persistence."""
...
def run_once(payload: dict[str, Any], project_root: Path, checkpoint: Path | None = None, state_path: Path | None = None) -> dict[str, Any]:
    ...
    persistence = load("devos_runtime_persistence", "tools/devos-runtime-persistence.py")
    ...
    output = {"loop_version":"P12-AUTO-LOOP-v2","status":accepted.get("status","FAILED"),"controller":controller_result,"handoff":handoff,"runtime":result,"accepted":accepted}
    if state_path is not None and result.get("evidence"):
        try:
            output["persistence"] = persistence.persist(state_path, result, str(task["id"]), str(task["objective"]))
        except (OSError, ValueError) as exc:
            output["status"] = "FAILED"
            output["persistence_error"] = str(exc)
    elif state_path is not None:
        output["status"] = "FAILED"
        output["persistence_error"] = "RAW_EXECUTION_EVIDENCE_REQUIRED"
    return output

def main() -> int:
    parser = argparse.ArgumentParser(description="Run one bounded DevOS autonomous iteration")
    parser.add_argument("input", type=Path); parser.add_argument("--project-root", type=Path, default=ROOT); parser.add_argument("--checkpoint", type=Path); parser.add_argument("--state", type=Path)
    args = parser.parse_args(); result = run_once(json.loads(args.input.read_text(encoding="utf-8")), args.project_root, args.checkpoint, args.state)
