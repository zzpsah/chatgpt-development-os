...
1. Start with the repository [AGENTS.md](../../AGENTS.md) and [manifest](../../.ai/manifest.yaml).
   For a fresh external AI chat, use the plain [Project Context Guide](../../DEVOS-PROJECT-CONTEXT.md) before this deeper handoff. After the recovery checklist is genuinely satisfied, use the DevOS activation handshake from that guide: stable `DEVOS MODE: ACTIVE` marker plus a short varied DevOS-only conversational confirmation. If recovery is incomplete or contradictory, use `DEVOS MODE: NOT VERIFIED` instead.
...
The activation handshake is only a context-recovery indicator. It never grants execution, merge, deployment, mutation, credential, permission, or production authority.
