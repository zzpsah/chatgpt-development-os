...
If a planned envelope is tampered so that contradictory/unknown claims are hidden by changing only top-level status metadata, P17 fails closed because it recomputes claim confidence consistency from the preserved resolver provenance.
